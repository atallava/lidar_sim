// hg sim scans looked pretty off. why? simulate packets for a single scan here
...
int main(int argc, char **argv)
{
    ...
    // todo: some limited number for debug
    size_t n_packets = 301;
...
#pragma omp for
    ...
    bool debug = true; 
    // write sim packets
    algo_state_est::mkdirsForPacketsToProcessDebug(section_scans_id, scans_version, 
						   sim_type, sim_version);
    std::string rel_path_sim_packets = 
    	algo_state_est::genRelPathPacketsToProcess(section_scans_id, scans_version, sim_type, sim_version, debug);
    ...
    std::string rel_path_real_pts = 
    	algo_state_est::genRelPathRealPtsRef(section_scans_id, scans_version, sim_type, sim_version, debug);
    ...
    std::string rel_path_sim_pts = 
    	algo_state_est::genRelPathSimPts(section_scans_id, scans_version, sim_type, sim_version, debug);
    writePtsToXYZFile(sim_pts, rel_path_sim_pts);

    // write sim detail
    std::string rel_path_sim_detail = 
    	algo_state_est::genRelPathSimDetail(section_scans_id, scans_version, sim_type, sim_version, debug);
