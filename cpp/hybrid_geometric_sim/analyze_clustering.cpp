...
int main(int argc, char **argv)
{
    ...
    // params to tune: n clusters per pt
    // min pts per cluster = 5. keep this as is
    double n_clusters_per_pt = 1000/(double)12016;;
    ...
    rel_path_ellipsoids = "data/analyze_clustering/ellipsoids.txt";
    ...
    {
	size_t id = section_pt_ids_to_process[i];
    ...
    // write the sim and real pts to file
    ...
    std::tie(asym_loss_sim_real_mean, asym_loss_sim_real_var) = metric.calcAsymmetricError(real_pts, sim_pts);
    std::cout << "asym_loss_real_sim_mean: " << asym_loss_real_sim_mean << " asym_loss_real_sim_var: " << asym_loss_real_sim_var << std::endl;
