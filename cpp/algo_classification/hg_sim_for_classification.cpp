...
#pragma omp for
    ...
    // write sim detail
    // barrels
    std::string path_sim_detail_for_barrels_dir = lsc::genPathSimDetailForObjectsDir(run_name, sim_type, sim_version, "scene_barrel");
    boost::filesystem::create_directories(path_sim_detail_for_barrels_dir);
    for (size_t i = 0; i < n_scene_barrels; ++i)
    {
    	std::string rel_path_sim_detail = lsc::genPathSimDetailForObject(run_name, sim_type, sim_version, "scene_barrel", i);
    ...
    // negatives
    std::string path_sim_detail_for_negatives_dir = lsc::genPathSimDetailForObjectsDir(run_name, sim_type, sim_version, "scene_negative");
    boost::filesystem::create_directories(path_sim_detail_for_negatives_dir);
    for (size_t i = 0; i < n_scene_negatives; ++i)
    {
    	std::string rel_path_sim_detail = lsc::genPathSimDetailForObject(run_name, sim_type, sim_version, "scene_negative", i);
