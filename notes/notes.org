...
- sim memory leak
  ...
  - seems like both error messages are from flann. marius says the bug was
    fixed, and i have version 1.8.4, but still seem to be getting the valgrind
    bug
  - create a version where you load a bunch of object meshes only, say the
    first 10. then load a section, and pass the same say first packet a couple
    of times to sim. see if you get new errors. 
...
- more top eyeballing. section 4, scans version 300118. (min, mem%).
  - (45, 55.4). (200, 56). (417, 57.8). (642, 58.5). (1000, 61). (1243, 63.2).
    ...
    i compiled, maybe some cleanup is done. (7746, 54.3).

- profiling
  - perf, operf for who is taking long etc
  - look up nvidia optix for raytracing

- look new valgrind report
- then rerun valgrind
- continue with gascola
- perhaps install caturana? give it a try before you go speaking to him.

- create flann matrix without new: change in dataproc utils, flann wrapper
- give stl vectors directly to knn/radius search
