...
- debug odoscan
  - scans in world frame look fine
  - lookup laser tf, and then check that laser frame data looks correct
  - viz and check in rviz too
  - check the velocity messages calculation being written to bag
  - what kind of output would you expect?
  - then the velocity pred output in rviz, what it looks like

- aggregating scans version 300118 took 1079s = ~18min.

- looking at scans in world frame and laser Tfs in matlab. they look right. the
  laser tfs lie along the path with correct orientation. they are also roughly
  in the middle of their respective scans
- bug! the laser scan rotations are off by pi/2 degrees, why? caught the bug. heh.
