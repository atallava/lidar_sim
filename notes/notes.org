...
- ask humphrey how he stores metadata, or format information
- process info format
  - source section file
  - n scans
  - packets per scan
  - skip between scans
- the aggregator tool
  - reads in the packets as a section
  - has the details from the process info
  - scan in world frame
  - scan in sensor frame
  - traj list of timestamp, pose pairs

- seems like there is cpp and py api to write rosbag. but can i make code
  outside catkin that uses rosbag? if not, am i forced to write a 'node' just
  for some data processing?
- check using rospy, in which case will do via rospy. then issue is about
  getting scans in correct data format
- i believe odoscan cloud message type is pcl::PointCloud<pcl::PointXYZ>. why
  did humphrey then say pointcloud2?
- first just load scan in python. try both, pcl and pointcloud2, and see what
  happens.

- test mm sim's capacity
- work the new variables into the calculation

- scrutinize bag_replayer code
- then conversion into rosbags
- read out rosbag scans to see if they make sense
- odoscan part of launch, read some odoscan
- throw into odoscan
- tool to read out the poses
- visualize the estimated trajectory

- i see, so packages can be in subdirs, what matters is having the
...
- launch nodes for rosbag and odoscan
- need to convert my data -> pcl -> rosbag. use py tools
- use service call to start rosbag playback. the StartPlayback service doesn't
  take any arguments
- load bag once, playback multiple times. cannot pause and reset bag once it
  starts playing, need to kill node. for finer control, better to set start and
  duration
- to automate experiments, use rosservice py api
- odoscan services have lo_node base
- argus_utils for what to set
- need to write out the published delta transforms to reconstruct estimated
  trajectory
- odoscan matcher has a buffer size, and max dt settin
- can look at laser geom ros package for help stitching scans together
...
- after installing humhu, did rospack profile.
